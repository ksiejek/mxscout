    ...
    return [
      { id: 'what', title: 'What MxScout is', blocks: [
        ...
        { kv: [
          ...
          ['Size', 'About 17,100 lines across 27 files — small enough to read end to end. The test suite ships alongside it in test/, another 36 files and about 4,800 lines that never run as part of the app'],
      ...
      { id: 'source', title: 'Reading the source', blocks: [
        ...
        { kv: [
          ...
          ['public/perf.js', 'The performance recording, browser side: the tiny PowerShell script that only reveals the admin password, the three setup steps, Start/Finish, a dashboard of saved recordings, and an analyzer (Overview / Timeline / Call tree / Hotspots tabs) for one of them — the Timeline stretches to as many pixels per second as you ask for and scrolls, and carries the runtime’s heap and database counters on the same ruler as the requests, all of it computed in this browser from samples already in its own database — the same comments.js findings every object popup uses, reachable from a Hotspots row. Recordings can be exported to a JSON file and imported back from one; import is a file read in this tab and a row written to this browser’s database, with no admin port, no server round trip and no network of any kind. It never talks to the admin port itself — it cannot, and the reason is in “Reading a running app’s admin port” above.'],
